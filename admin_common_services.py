...
def send_reminder_for_token(mySQLCursor, sysDict):
    selectStatment = """SELECT UTA.NAME_IN_BROKER, UTA.TRADE_ACCOUNT, UTA.TELEGRAM_USER_NAME FROM USR_TRADE_ACCOUNTS UTA    
                        LEFT JOIN USR_STRATEGY_SUBSCRIPTIONS USS ON USS.TRADE_ACCOUNT= UTA.TRADE_ACCOUNT 
                        WHERE (UTA.ACTIVE_FLAG='Y' AND UTA.ACCOUNT_TYPE = 'LIVE' AND UTA.ACCESS_TOKEN_VALID_FLAG = 'N' AND USS.ACTIVE_FLAG ='ACTIVE')
                        OR (UTA.ACCESS_TOKEN_VALID_FLAG = 'N' AND UTA.TRADE_ACCOUNT IN (SELECT TRADE_ACCOUNT FROM TRADE_TRANSACTIONS WHERE TRADE_STATUS IN ('OPEN') GROUP BY TRADE_ACCOUNT))
                        GROUP BY UTA.TRADE_ACCOUNT"""
    ...
    for row in results:
        name = row[0]
        accountId = row[1]
        sysDict['telegramAdminIds'] = row[2]
        alertMsg = f"Dear {name}, This is a kind reminder for adding TOTP in AlphaGain website for your account {accountId}."
        util.add_logs(cnx, mySQLCursor, 'NOTIFY', alertMsg, sysDict)
    ...
    while programExitFlag:
        try:
            ...
            if (Config.TESTING_FLAG or (int(currentTime) <= int(sysSettings['SYSTEM_UPDATE_SVC_END_TIME']))):
                ...
                send_reminder_for_token(mySQLCursor, sysDict)
